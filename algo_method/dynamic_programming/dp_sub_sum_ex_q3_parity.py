...
class Solution:
    ...
    def solve(self):
        ...
        # dp[i][j][p]: ball i, weight sum j,
        # parity: {0: Num of balls selected from the first i balls is even,
        #          1: Num of balls selected from the first i balls is odd}
        dp = [[[0] * 2 for _ in range(M+1)] for _ in range(N+1)]
        dp[0][0][0] = 1
        for i in range(N):
            for weight in range(M+1):
                for parity in range(2):
                    if dp[i][weight][parity] == 0:
                        # There is no way to reach (i, j, p)
                        continue
                    # Not use ball i. Parity does not change.
                    dp[i+1][weight][parity] += dp[i][weight][parity]
                    # Use ball i. Parity changes.
                    if weight + W[i] <= M:
                        dp[i+1][weight+W[i]][parity ^ 1] += dp[i][weight][parity]
