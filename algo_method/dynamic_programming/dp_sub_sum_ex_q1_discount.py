...
class Solution:
    ...
    def solve(self):
        ...
        INF = float('inf')
        # dp[i][j]: Minimum sum of prices when I go to store j on day i.
            ...
            for s in range(3):
                for ps in range(3):
                    ...
                    if ps == s:
                        dp[i][s] = min(
                            dp[i][s],
                            dp[i-1][ps] + prices[s][i] - A)
                    else:
                        dp[i][s] = min(dp[i][s], dp[i-1][ps] + prices[s][i])
...
class Try:
    ...
    def solve(self):
        ...
        for i in range(1, N):
            for j in range(3):
                dp[i][j] = prices[i][j]
                min_prices = dp[i-1][j] - A
                for s in range(3):
                    if s == j:
                        continue
                    min_prices = min(min_prices, dp[i-1][s])
                dp[i][j] += min_prices
